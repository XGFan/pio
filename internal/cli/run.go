...
import (
	...
	"github.com/guofan/pio/internal/latency"
	syncpkg "github.com/guofan/pio/internal/sync"
	"github.com/guofan/pio/internal/tunnel"
	"github.com/guofan/pio/internal/web"
...
func runDaemon(ctx context.Context, deps Deps, args []string) int {
	...
	if v := os.Getenv("PIO_PROXY_BIND"); v != "" {
		settings.ProxyBind = v
	}
	if v := os.Getenv("PIO_PROXY_AUTOSTART"); v == "true" || v == "1" {
	...
	fmt.Fprintf(deps.Stdout, "piod run: api=127.0.0.1:%d  proxy_running=%v  proxy=%s\n",
