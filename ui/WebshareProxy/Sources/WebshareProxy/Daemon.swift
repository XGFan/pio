// Daemon.swift — Spawn the bundled `webshare-proxyd run` binary as a
...
final class DaemonSupervisor {
    ...
    init() {
        ...
        self.dataDir = support.appendingPathComponent("webshare-proxy", isDirectory: true)
    ...
    func start() throws {
        ...
        // dev users `cp webshare-proxyd ./build/`).
        let exe = locateDaemonBinary()
        guard let exe else {
            throw NSError(domain: "WebshareProxy", code: 1, userInfo: [
                NSLocalizedDescriptionKey: "Could not locate webshare-proxyd binary. Run scripts/build-app.sh first."
            ])
        }
    ...
    func waitForAPIPort(timeout: TimeInterval = 10) throws -> Int {
        ...
        throw NSError(domain: "WebshareProxy", code: 2, userInfo: [
    ...
    static func log(_ s: String) {
        ...
        if let data = line.data(using: .utf8) {
            let url = URL(fileURLWithPath: "/tmp/webshare-proxy.log")
    ...
    private func locateDaemonBinary() -> URL? {
        ...
        if let main = Bundle.main.executableURL {
            let sibling = main.deletingLastPathComponent().appendingPathComponent("webshare-proxyd")
            if FileManager.default.isExecutableFile(atPath: sibling.path) {
                return sibling
            }
        }
        // 2. WEBSHARE_PROXYD_BINARY env override for development.
        if let env = ProcessInfo.processInfo.environment["WEBSHARE_PROXYD_BINARY"],
           FileManager.default.isExecutableFile(atPath: env) {
            return URL(fileURLWithPath: env)
        }
        // 3. ./webshare-proxyd next to the cwd.
        let cwd = URL(fileURLWithPath: FileManager.default.currentDirectoryPath)
            .appendingPathComponent("webshare-proxyd")
